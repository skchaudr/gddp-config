...
comes from jobs + executor_sessions + results. Nothing here mutates anything;
rerun after a human acceptance and the frontier simply recomputes.
"""
...
def _latest_verdict(con, job_id: str):
    try:
        row = con.execute(
            "SELECT outcome FROM results WHERE job_id = ? "
...
def load_runtime(con, project_id: str) -> dict:
    """node_id -> {phase, job_id, verdict} for nodes whose latest job is open."""
    active = {}
    for node_id, job in _latest_job_per_node(con, project_id).items():
        ...
        active[node_id] = {
            "phase": phase,
            "job_id": job["job_id"],
            "verdict": _latest_verdict(con, job["job_id"]),
        }
    return active
...
def derive(graph: dict, runtime: dict) -> dict:
    ...
    for node_id, info in sorted(graph.items()):
        ...
        if motion is not None:
            ...
            ready.append((node_id, info["executor"], unsatisfied))
    for node_id, info in sorted(graph.items()):
        if info["status"] != "pending":
            continue
        deps = [
            (dep, (graph.get(dep) or {}).get("status") or "missing")
            for dep in info["depends_on"]
        ]
        incomplete = [(d, s) for d, s in deps if s not in SATISFIED_DEP_STATUSES]
        blocked.append((node_id, incomplete))
    ...
    for node_id, motion in sorted(runtime.items()):
        ...
        for cand, info in sorted(graph.items()):
            if info["status"] != "pending" or node_id not in info["depends_on"]:
                continue
...
def render_text(project_id: str, derived: dict) -> str:
    lines = [f"frontier: {project_id}"]
    counts = (
        f"  {len(derived['ready'])} ready · "
        f"{len(derived['in_flight'])} in flight · "
        f"{len(derived['blocked'])} pending · "
        f"{len(derived['unlocks'])} acceptance unlocks"
    )
    lines.append(counts)

    lines.append("\nready now (dispatchable):")
    for node_id, executor, unsatisfied in derived["ready"]:
        line = f"  {node_id}" + (f"  [{executor}]" if executor else "")
        if unsatisfied:
            line += f"  (dep not complete: {', '.join(unsatisfied)})"
        lines.append(line)
    ...
    for node_id, motion in derived["in_flight"]:
        verdict = f"  verdict: {motion['verdict']}" if motion["verdict"] else ""
        lines.append(f"  {node_id}  — {motion['phase']}{verdict}")
    if not derived["in_flight"]:
        lines.append("  (none)")

    lines.append("\nblocked (incomplete dependencies):")
    for node_id, incomplete in derived["blocked"]:
        if incomplete:
            detail = ", ".join(f"{dep} [{status}]" for dep, status in incomplete)
            lines.append(f"  {node_id}  ← {detail}")
        else:
            lines.append(f"  {node_id}  ← deps satisfied; graph status still pending")
    if not derived["blocked"]:
        lines.append("  (none)")

    lines.append("\nunlocks on acceptance:")
    for node_id, downstream in derived["unlocks"]:
        lines.append(f"  accept {node_id} → {', '.join(downstream)}")
    if not derived["unlocks"]:
        lines.append("  (none)")
    return "\n".join(lines)


def connect_readonly(db_path: Path):
    """Immutable read handle; returns None when the runtime DB does not exist."""
