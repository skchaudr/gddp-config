...
| `jobs show <job-or-node> [--full]` | Show verdict, evaluator reasoning, findings, then job provenance |
...
- `node show` groups details under `OVERVIEW`, `STATUS`, `INTENT`, `GRAPH`, `DELIVERY CONTRACT`, `EVALUATION`, and optional `TRACE` dividers. The evaluation-only view leads with verdict, evaluator reasoning, findings, and next action; runtime/job metadata and overview follow as provenance.
